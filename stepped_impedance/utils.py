...
def get_dataset():
    ...
    """
    fp = 'Data/'
    data_a_in = pd.read_csv(fp + "STEPPED_LPF_4TH_combined.csv", encoding="gbk")
    data_b_in = pd.read_csv(fp + "STEPPED_LPF_5TH_combined-2f.csv", encoding="gbk")
    ...
    data_a['NUM'] = np.ones(len(data_a))
    data_a_share['NUM'] = np.ones(len(data_a_share))   
    data_b['NUM'] = 3*np.ones(len(data_b))
    data_b_share['NUM'] = 3*np.ones(len(data_b_share))
    ...
    if bool(args.test_unexampled):
        print("Test model with the unexampled 6th order filter data")
        # 测试数据使用6阶 "1" means unexampled data
        test = pd.read_csv(fp + "STEPPED_LPF_6TH_combined.csv", encoding="gbk")
        test['NUM'] = 3*np.ones(len(test))
        test = test.sample(frac=1, random_state=1)
        n = test.shape[0]
        test_a = test.iloc[:int(n / 2)]
        test_b = test.iloc[int(n / 2):]
    else: 
        print("Test model with the exampled 4th and 5th order filter")   
        # 测试数据使用4，5阶打乱版，数据打标签，混合, 分给A和B, 打乱
        # "0" mean exampled data
        test_a['NUM'] = np.ones(len(test_a))
        test_b['NUM'] = 2*np.ones(len(test_b))
